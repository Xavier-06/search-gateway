#!/usr/bin/env python3
"""
统一搜索网关 - Search Gateway for WorkBuddy

搜索栈：
  Layer 1: DuckDuckGo Python API 直连（主力，中英文兼顾）
  Layer 2: SearXNG 本地实例（Baidu/Bing 补充）
  Layer 3: Google 直接抓取（需要代理）
...
SearXNG 安装：
  - Docker: docker run -d -p 8888:8888 --name searxng searxng/searxng:latest
  - 或参考 SETUP_GUIDE.md
"""
...
def _ddg_search(query: str, max_results: int = 10) -> list:
    ...
    try:
        if not os.path.exists(DDGS_BIN) and not shutil.which(DDGS_BIN):
            return []
        result = subprocess.run(
            [DDGS_BIN, "text", "-q", query, "-m", str(max_results + 5), "-r", "wt-wt"],
...
def _searxng_search(query: str, max_results: int = 10, engines: str = "", timeout: int = 25) -> list:
    ...
    try:
        r = _LOCAL.get(
            ...
            params=params,
            timeout=timeout,
            headers={"User-Agent": "SearchGateway/1.0"},
            verify=False,
        )
        r.raise_for_status()
        data = r.json()
...
def google_search(query: str, max_results: int = 10) -> list:
    ...
    try:
        ...
        results = []
        for m in re.finditer(r'<a\s+href="/url\?q=([^&"]+)&[^"]*"[^>]*>(.*?)</a>', r.text):
            href = m.group(1)
...
def yfinance_summary(ticker: str) -> Optional[dict]:
    """获取上市公司估值快照（需要走代理访问 Yahoo Finance）。"""
    try:
...
def search(query: str, max_results: int = 10, timeout: int = 25, prefer: str = "auto") -> list:
    ...
    prefer:
        auto    - DDG 优先 + SearXNG 补充（默认）
        ddg     - 只用 DDG
        searxng - 只用 SearXNG
        google  - 只用 Google（需要代理）
        multi   - DDG + SearXNG + Google 三路合并
    """
    if prefer == "ddg":
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Search Gateway")
